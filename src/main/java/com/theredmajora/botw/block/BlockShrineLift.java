    	...
    	if(!worldIn.isRemote)
    	{
	    	if(entityIn instanceof EntityPlayerMP)
	    	{
	    		...
	    		if(player.isSneaking())
	    		{
	    			...
	    			if(player.dimension==BOTWDimensions.SHRINE.getId())
	    			{
	    				worldIn.getMinecraftServer().getPlayerList().transferPlayerToDimension(player, 0, new TeleporterShrine(worldIn.getMinecraftServer().worldServerForDimension(0), (TileEntityShrineLift) worldIn.getTileEntity(pos)));
	        		}
	    			else
	    			{
